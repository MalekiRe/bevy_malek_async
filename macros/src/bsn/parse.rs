use crate::bsn::types::{
    ...
    BsnNamedField, BsnRelatedSceneList, BsnRoot, BsnSceneList, BsnSceneListItem, BsnSceneListItems,
    BsnTuple, BsnType, BsnUnnamedField, BsnValue,
};
use proc_macro2::{Delimiter, TokenStream, TokenTree};
use quote::quote;
use syn::{
    Block, Expr, Ident, Lit, LitStr, Path, Result, Token, braced, bracketed,
    buffer::Cursor,
    parenthesized,
...
impl BsnEntry {
    fn parse(input: ParseStream, found_inherited_scene: bool) -> Result<Self> {
        Ok(if input.peek(Token![:]) {
            ...
            BsnEntry::AsyncClosure(BsnAsyncClosure {
                tokens: parse_async_closure_loose(input)?,
            })
...
fn parse_async_closure_loose(input: &ParseBuffer) -> Result<TokenStream> {
            ...
            match &tt {
                TokenTree::Punct(punct) if punct.as_char() == '|' => {
                        ...
                        && group.delimiter() == Delimiter::Brace
                    {
                        ...
                        return Err(cursor.error("async closures expect '{' to follow '|'"));
                    }
                }
                _ => rest = next,
            }
...
mod tests {
    ...
    use crate::bsn::types::{Bsn, BsnEntry};
    ...
    #[test]
    fn parses_async_closure_entry() {
        let bsn =
            parse_str::<Bsn<true>>("async |world: AsyncWorld| { let entity = #Foo; }").unwrap();

        assert!(matches!(
            bsn.entries.as_slice(),
            [BsnEntry::AsyncClosure(_)]
        ));
    }
}
